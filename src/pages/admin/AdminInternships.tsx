...
export default function AdminInternships() {
  ...
  useEffect(() => {
    void Promise.all([listAdminInternships(), listAdminApplications()])
      .then(([jobs, nextApplications]) => {
        setItems(jobs);
  ...
  return <AdminLayout title="求人管理">
    <div className="adminToolbar"><p>求人の登録、編集、公開状態の変更を行います。</p><Link className="careerPrimaryButton" to="/admin/internships/new">新しい求人を登録</Link></div>
    {message && <p className="formSuccess" role="status">{message}</p>}
    {error && <p className="formError" role="alert">{error}</p>}
    {loading ? <div className="careerState">読み込んでいます...</div> : items.length === 0 ? <div className="careerState">登録された求人はありません。</div> :
      <div className="adminTableWrap"><table className="adminTable"><thead><tr><th>求人</th><th>職種</th><th>公開状態</th><th>応募件数</th><th>締切</th><th>作成日</th><th>操作</th></tr></thead><tbody>
